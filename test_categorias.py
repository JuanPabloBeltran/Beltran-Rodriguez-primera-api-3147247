...
def test_listar_categorias_con_datos(client: TestClient):
    client.post("/categorias/", json={"nombre": "Libros", "descripcion": "Libros y literatura"})
    response = client.get("/categorias/")
    data = response.json()
    assert len(data) == 1
...
def test_obtener_categoria(client: TestClient):
    create_response = client.post("/categorias/", json={"nombre": "Deportes", "descripcion": "Artículos deportivos"})
